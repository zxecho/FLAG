...
# Load federated model or local models
...
# load each local model
def load_local_models(n_p, save_file):
    for i in range(n_p):
        g_load_path = './saved_model/' + save_file + '/independent_' + str(i) + '_G.pkl'
...
def get_ACGAN_synthetic_samples(G, size, latent_dim, n_classes, labels_stat, device):
    """
    用AC GAN 生成的数据
    :param G:
    :param size:
    ...
    :return:
    """
    # Set different labels with different sampling prob
    all_labels = [i for i in range(n_classes)]
    lack_labels = list(set(all_labels) - set(labels_stat))
    label_select = all_labels  # all_labels / lack_labels / label_stat
    bs = size * len(label_select)
    noise = torch.randn(bs, latent_dim)

    # lack_labels_p = [0.8/len(lack_labels) for _ in range(len(lack_labels))]
    # stat_labels_p = [0.2/len(labels_stat) for _ in range(len(labels_stat))]

    ''' 随机采样方法
    labels_p = [round(0.9/len(lack_labels), 2) for _ in range(len(lack_labels))]
    labels_p.extend([round(0.1/len(labels_stat), 2) for _ in range(len(labels_stat))])
    if sum(labels_p) != 1.0:
        err = 1 - sum(labels_p)
        labels_p[0] += err
        labels_p[0] = round(labels_p[0], 2)
    assert round(sum(labels_p), 2) == 1.0, print('label p sum: ', sum(labels_p), 'labels p: \n', labels_p)
    labels = []
    labels.extend(lack_labels)
    labels.extend(labels_stat)

    random_sample_lbs = np.random.choice(labels, size=size, p=labels_p)
    '''
    # 均匀分布
    sample_lbs = []
    for lb in label_select:
        temp_l = [lb] * size
        sample_lbs.extend(temp_l)

    sample_labels = torch.as_tensor(sample_lbs, dtype=torch.int64, device=device)
...
def get_labels_stat(dataset_name, n_classes, training_dataset_name):
    dataset_files = os.listdir('./dataset/{}'.format(dataset_name))
    lb_counter_list = [0 for _ in range(n_classes)]
    for file in dataset_files:
        if training_dataset_name in file and 'stat' in file:
            each_family_label_stat = loadjson('./dataset/{}'.format(dataset_name), file)
    for fam_name, lb_stat in zip(each_family_label_stat.keys(), each_family_label_stat.values()):
        fam_lb_temp_list = []
        for lb, lb_n in enumerate(lb_stat):
            if lb_n != 0:
                lb_counter_list[lb] += 1
                fam_lb_temp_list.append(lb)

        each_family_label_stat[fam_name] = fam_lb_temp_list

    return each_family_label_stat, lb_counter_list


# 用于记录全局权重变化
def get_global_lb_weights(client, lb_stat, weights_list, cur_client_lb_dict):
    lb_list = lb_stat[client]
    tem_lb_dict = {}
    for lb in lb_list:
        if client not in cur_client_lb_dict.keys():
            cur_client_lb_dict[client] = dict()
        weights_list[lb] += 1
        cur_client_lb_dict[client][lb] = weights_list[lb]

    return weights_list, cur_client_lb_dict
