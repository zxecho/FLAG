...
from Fed_Local_train import Local_ACGAN_Train, IdptTraining, Local_C_with_DA_Train
...
def fed_main(args, save_path=''):
    ...
    if args.init_FedD_with_C:
        ...
        if not args.idpt_usrs_training:
            ...
            idpt_pretrained_model = file_name[2]
    ...
    for fed_iter in range(args.epochs):  # 暂时取消self.args.local_ep *
        ...
        for idx in idxs_users:
            ...
            train_no = Local_ACGAN_Train(args,
...
if __name__ == "__main__":
    ...
    params_test_list = [True, False]  # 用于设置不同参数
    ...
    for param in params_test_list:
        ...
        # 用于多参数实验
        # args.exp_name = 'FedDA_ACGAN_{}_FedG_MixAllLalels_New_v2_T2({}{})'.format(args.training_dataset_name,
        #                                                                    test_param_name, param)
        args.exp_name = 'FedDA_ACGAN_{}_FedG_MixAllLalels_Test'.format(args.training_dataset_name)
        ex_params_settings = {
            'algo_name': 'FedDA',
            # GAN算法相关参数
