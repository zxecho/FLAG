...
from Fed_Local_train import Local_vanilla_ACGAN_Train, IdptTraining, Local_C_with_DA_Train
...
def fed_main(args, save_path=''):
    ...
    for fed_iter in range(args.epochs):  # 暂时取消self.args.local_ep *
        ...
        for idx in idxs_users:
            ...
            train_no = Local_vanilla_ACGAN_Train(args,
                                                 globalGModel=deepcopy(FedGNet),
                                                 globalDModel=FedDNet if args.if_Fed_train_D else family_DNets_list[
                                                     idx],
...
if __name__ == "__main__":
    ...
    for param in params_test_list:
        ...
        args.exp_name = 'FedDA_vanilla_ACGAN_{}_MixAllLalels_baseline_T2'.format(args.training_dataset_name)
        ex_params_settings = {
            'algo_name': 'FedDA',
            # GAN算法相关参数
